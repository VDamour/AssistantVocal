...
def getTodayWeatherResponse(city):
    ...
    response = "Aujourd'hui à " + city + " la température est de " + str(int(weather['main']['temp'])) + " degrés celsius " \
                "avec un taux d'humidité de "+ str(weather['main']['humidity']) +"% " \
                "et une pression atmosphérique de "+ str(int(weather['main']['pressure'])) +" hectopascal."
    return response

def getTomorrowWeatherResponse(city, timestamp):
    weather = getTomorrowWeather(city, timestamp)
    response = "Demain à " + city + " la température prévu est de " + str(int(weather['main']['temp'])) + " degrés celsius " \
                "avec un taux d'humidité de "+ str(weather['main']['humidity']) +"% " \
                "et une pression atmosphérique de "+ str(int(weather['main']['pressure'])) +" hectopascal."
    return response
...
#print(getWeatherResponse('Paris', '2018-06-14T12:00:00+04:00'))
